...
public class AlumnosTest {
	...
	public void constructorCapacidadValidaCreaAlumnosCorrectamente() {
		...
		assertThat(ALUMNOS_NO_CREADOS, alumnos.getCapacidad(), is(5));
	...
	public void constructorCapacidadNoValidaLanzaExcepcion() {
		...
		try {
			...
			assertThat(OBJETO_DEBERIA_SER_NULO, alumnos, is(nullValue()));
		} catch (Exception e) {
	...
	public void insertarAlumnoValidoConAlumnosVaciosInsertaAlumnoCorrectamente() {
		...
		try {
			...
			assertThat(REFERENCIA_NO_ESPERADA, copiaAlumnos[0], not(sameInstance(alumno1)));
			assertThat(OPERACION_NO_REALIZADA, copiaAlumnos[0], is(alumno1));
	...
	public void insertarDosAlumnosValidosInsertaAlumnosCorrectamente() {
		...
		try {
			...
			assertThat(REFERENCIA_NO_ESPERADA, copiaAlumnos[1], not(sameInstance(alumno2)));
			assertThat(OPERACION_NO_REALIZADA, copiaAlumnos[1], is(alumno2));
		} catch (OperationNotSupportedException e) {
			fail(EXCEPCION_NO_PROCEDE);
		}
	}
	...
	public void insertarTresAlumnosValidosInsertaAlumnosCorrectamente() {
		...
		try {
			...
			assertThat(REFERENCIA_NO_ESPERADA, copiaAlumnos[2], not(sameInstance(alumno3)));
			assertThat(OPERACION_NO_REALIZADA, copiaAlumnos[2], is(alumno3));
		} catch (OperationNotSupportedException e) {
			fail(EXCEPCION_NO_PROCEDE);
		}
	}
	...
	@Test
	public void insertarAlumnoValidoConAlumnosLlenosLanzaExcepcion() {
		...
		try {
			...
			assertThat(MENSAJE_NO_CORRECTO, e.getMessage(), is(ERROR_NO_MAS_ALUMNOS));
			assertThat(TAMANO_NO_ESPERADO, alumnos.getTamano(), is(2));
			assertThat(ALUMNO_NO_ESPERADO, alumnos.buscar(alumno1), is(alumno1));
			assertThat(REFERENCIA_NO_ESPERADA, alumnos.buscar(alumno1), not(sameInstance(alumno1)));
			assertThat(OPERACION_NO_REALIZADA, alumnos.get()[0], is(alumno1));
			assertThat(ALUMNO_NO_ESPERADO, alumnos.buscar(alumno2), is(alumno2));
